...
class SkipTimelineSegmentsTest {
    @Test
    fun segmentsAreClampedToDuration() {
        val markers = listOf(
    ...
    @Test
    fun emptyRangesAndSegmentsOutsideDurationAreIgnored() {
        val markers = listOf(
    ...
    @Test
    fun knownPositiveDurationIsRequired() {
        val segment = VideoSkipSegment(VideoSkipKind.Opening, 10_000L, 90_000L)

        assertEquals(emptyList(), listOf(segment).timelineMarkerSegments(durationMs = null))
        assertEquals(emptyList(), listOf(segment).timelineMarkerSegments(durationMs = 0L))
    }
}
