...
// UiText
@Composable
internal fun uiText(key: UiStringKey, vararg formatArgs: Any): String = uiText(key.resId, *formatArgs)

@Composable
internal fun uiText(@StringRes resId: Int, vararg formatArgs: Any): String {
    val context = LocalContext.current
    val language = LocalUiLanguage.current
    return remember(context, language, resId, formatArgs.toList()) {
...
internal fun localizedPluralWord(
    ...
    return uiText(pluralWordKey(count, LocalUiLanguage.current, one, few, many))
}

internal fun pluralWordKey(
    count: Long,
    ...
    if (language == ContentLanguage.English) return if (count == 1L || count == -1L) one else many
    val mod100 = kotlin.math.abs(count % 100)
    val mod10 = mod100 % 10
    return when {
...
internal enum class UiStringKey(
    ...
    DoNotAdd(R.string.ui_do_not_add),
    ...
    LocalWatchHistoryMergeMessage(R.string.ui_local_watch_history_merge_message),
    LocalWatchHistoryMergeTitle(R.string.ui_local_watch_history_merge_title),
    ...
    SupplementProfile(R.string.ui_supplement_profile),
