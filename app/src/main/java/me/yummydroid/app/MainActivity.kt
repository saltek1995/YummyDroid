...
abstract class MainActivityRuntime : FragmentActivity() {
    ...
    private lateinit var attachedSettings: AppSettings

    override fun attachBaseContext(newBase: Context) {
        attachedSettings = AppSettingsStorage(newBase).read()
        super.attachBaseContext(
            newBase.withAppUiConfiguration(
                interfaceScale = attachedSettings.interfaceScale,
                contentLanguage = attachedSettings.contentLanguage,
            ),
        )
    }
    ...
    private fun observeUiSettings() {
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                AppSettingsStorage(this@MainActivityRuntime).observe()
                    .flowOn(Dispatchers.IO)
                    .first { it.requiresActivityRecreation(attachedSettings) }
