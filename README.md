...
Обычный `check` не запускает многоминутное воспроизведение в реальном времени.
Эти сценарии используют локальные видеофайлы и тестовый HTTP-сервер и запускаются отдельно:

```powershell
.\gradlew.bat :app:testDebugUnitTest -PrunRealtimePlaybackTests=true --tests '*RealTimeBufferingIntegrationTest'
```

Проверки реальных провайдеров находятся в instrumentation-тестах и требуют
отдельного явного запуска с параметрами источника. В проверку релиза они не входят.
